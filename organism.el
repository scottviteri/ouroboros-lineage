           ...
           (while (progn
             ...
             (let ((form (read (current-buffer))))
               ...
               (unless (memq (car form)
                             '(require defun defmacro defvar defconst defcustom
                               condition-case provide))
                 (error "Unsafe top-level form"))
               (setq forms (1+ forms))
               (when (and (eq (car form) 'require)
                          (equal (cadr form) '(quote json)))
                 (setq requires-json t))
               (when (and (eq (car form) 'require)
                          (equal (cadr form) '(quote subr-x)))
                 (setq requires-subr-x t))
